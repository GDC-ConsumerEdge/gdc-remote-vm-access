...
# 0. Install Node.js manually with checksum verification (replaces curl | bash)
# Also install git, procps, and gke-auth-plugin
...
# 1. Download specific kubectl version (v1.30.0) and verify checksum
RUN curl -LO "https://dl.k8s.io/release/v1.30.0/bin/linux/amd64/kubectl" && \
    echo "7c3807c0f5c1b30110a2ff1e55da1d112a6d0096201f1beb81b269f582b5d1c5  kubectl" | sha256sum -c - && \
    chmod +x kubectl && \
    mv kubectl /usr/local/bin/

# 2. Download virtctl (Pinned to a specific stable version) and verify checksum
...
# 3. Clone noVNC (Pinned to tag v1.7.0)
